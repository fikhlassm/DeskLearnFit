...
class MateriTest extends TestCase
{
    ...
    {
        [$pengajar, $kelas] = $this->setupKelasWithSiswa();
        $materi = Materi::factory()->create(['kelas_id' => $kelas->id, 'pengajar_id' => $pengajar->id]);
        ...
        $this->assertDatabaseHas('materi', ['id' => $materi->id, 'judul' => 'Judul Baru']);
    }
    ...
    {
        [$pengajar, $kelas] = $this->setupKelasWithSiswa();
        $materi = Materi::factory()->create(['kelas_id' => $kelas->id, 'pengajar_id' => $pengajar->id, 'status' => 'draf']);

        $this->actingAs($pengajar)
            ->patch("/dashboard/materi/{$materi->id}/publish")
            ->assertRedirect(route('materi.index', $kelas->id));

        $this->assertDatabaseHas('materi', ['id' => $materi->id, 'status' => 'terbit']);
    }
    ...
    public function test_siswa_bisa_melihat_materi_terbit_dari_kelas_yang_diikuti(): void
    {
        ...
        $materi = Materi::factory()->terbit()->create(['kelas_id' => $kelas->id, 'pengajar_id' => $pengajar->id]);
    ...
    {
        [$pengajar, $kelas, $siswa] = $this->setupKelasWithSiswa();
        Materi::factory()->create(['kelas_id' => $kelas->id, 'pengajar_id' => $pengajar->id, 'status' => 'draf', 'judul' => 'Materi Draf']);
