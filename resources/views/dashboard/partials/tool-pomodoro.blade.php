...
<div class="tool-pomodoro" data-sesi-id="{{ $sesi->id }}">
    ...
    <form method="POST" action="{{ route('sesi.catatan', $sesi) }}" class="tool-pomodoro__catatan">
        ...
        <textarea name="catatan" rows="3" maxlength="2000" placeholder="cth: Hari ini bahas integral parsial, masih bingung bagian trigonometric substitution..." {{ $sesi->status === 'selesai' ? 'readonly' : '' }}>{{ $sesi->catatan }}</textarea>
        @if($sesi->status !== 'selesai')
